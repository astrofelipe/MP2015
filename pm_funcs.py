...
def get_tlineal():
    ...
    local, ma1, ma2, mr1, mr2, mp1, mp2, rad_ref, x0, y0, lim, plot_del_ep, plot_del_xy = params['TLINEAL_1a1.PY'].values()
...
def get_CMD():
    cmd_modo, col1, col2, mag1, mag2 = params['CMD.PY'].values()
    col1, col2, mag1, mag2 = np.array([col1, col2, mag1, mag2]).astype(float)
    cmd_modo = str(cmd_modo)

    return cmd_modo, col1, col2, mag1, mag2
